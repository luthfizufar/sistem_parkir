$(document).ready(function(){
    ...
    $.ajax({
      ...
      success: function(response){
        ...
        if (response == 1) {
          ...
          $('.alert_user').html('<p class="alert alert-success">A new Vehicle has been successfully added</p>');
        }
        else{
          $('.alert_user').fadeIn(500);
    ...
    $.ajax({
      ...
      success: function(response){
        ...
        if (response == 1) {
          ...
          $('#dev_sel').val('0');
          $('.alert_user').fadeIn(500);
          $('.alert_user').html('<p class="alert alert-success">The selected Vehicle has been updated!</p>');
        }
        else{
          $('.alert_user').fadeIn(500);
          $('.alert_user').html('<p class="alert alert-danger">'+ response + '</p>');
  ...
  $(document).on('click', '.user_rmo', function(){
    ...
    bootbox.confirm("Do you really want to delete this vehicle?", function(result) {
      if(result){
        $.ajax({
          url: 'manage_users_conf.php',
          type: 'POST',
          ...
          success: function(response){
            ...
            if (response == 1) {
              ...
              $('#dev_sel').val('0');
              $('.alert_user').fadeIn(500);
              $('.alert_user').html('<p class="alert alert-success">The selected Vehicle has been deleted!</p>');
            }
            else{
              $('.alert_user').fadeIn(500);
              $('.alert_user').html('<p class="alert alert-danger">'+ response + '</p>');
    ...
    $.ajax({
      ...
      success: function(response){
       ...
       // $('#dev_sel').val(user_dev.User_dev);
        ...
        if (user_dev.User_dev == 'Faculty'){
          $('.form-style-5').find(':radio[name=dev_sel][value="Faculty"]').prop('checked', true);
      }
      else{
        $('.form-style-5').find(':radio[name=dev_sel][value="Students"]').prop('checked', true);
      }


        if (user_gender.User_gender == 'Four Wheeler'){
            $('.form-style-5').find(':radio[name=gender][value="Four Wheeler"]').prop('checked', true);
        }
        else{
            $('.form-style-5').find(':radio[name=gender][value="Two Wheeler"]').prop('checked', true);
        }
